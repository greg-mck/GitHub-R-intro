...
```{r}
fc <- filter(counties, NAME %in% c("Larimer" , "Denver" , "Pueblo"))
fc
```
...
```{r}
poudre_points_sf[nrow(poudre_points_sf)+1,] = c(extract(elevation, poudre_points_sf))


#mutate(poudre_points_prj, extract(elevation, poudre_points_sf))
poudre_points_sf
#ggplot(poudre_points_sf) +
  #geom_bar(mapping = aes(x = name, y = extract(elevation, poudre_points_sf)))
